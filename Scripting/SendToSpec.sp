...
public Action:ForceSendPlayerAFK(client, args)
{
	if(args > 2 || args  == 0)
	{
		CPrintToChat(client, "[SPEC] Usage : sm_fspec [TARGET] \'\'[optional:REASON]\'\'");
		return Plugin_Handled;
	}
	...
	if(target_count == 1)
	{
		CPrintToChatAll("[SPEC] Player %s moved to spectator, reason : '%s' !", target_name, arg2);
	...
	else
	{
		...
		CPrintToChatAll("[SPEC] Some players as been moved to spectator, reason : '%s' !", arg2);
	}
...
stock SendToSpec(client, bool:user_choice)
{
	...
	{
		CPrintToChat(client, "[SPEC] Only Humans can go to Spectate!");
	}
}