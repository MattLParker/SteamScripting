...
{
	name = "CT Ban",
	author = "databomb, Edited by Elusive",
	description = "Allows admins to ban players from joining the CT team.",
	version = PLUGIN_VERSION,
	url = "https://github.com/MattLParker/SteamScripting/"
};
...
public OnPluginStart()
{
	...
	RegConsoleCmd("sm_amibanned", Command_amibanned);
...
public Action:Command_amibanned(client, args)
{
	decl String:isBanned[3];
	GetClientCookie(client, g_CT_Cookie, isBanned, sizeof(isBanned));
	new banFlag = StringToInt(isBanned);	
	if (banFlag)
	{
		// find the time if any
		if (gA_LocalTimeRemaining[client] <= 0)
		{
			ReplyToCommand(client, CHAT_BANNER, "Permanent CT Ban", client);
		}
		else
		{
			ReplyToCommand(client, CHAT_BANNER, "Temporary CT Ban", client, gA_LocalTimeRemaining[client]);
		}
	}
	else
	{
		ReplyToCommand(client, CHAT_BANNER, "Not CT Banned", client);
	}
	return Plugin_Handled;
}
