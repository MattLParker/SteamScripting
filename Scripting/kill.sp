...
#define PLUGIN_NAME "Disarm/slay"
#define PLUGIN_URL "https://github.com/MattLParker/SteamScripting/"
#define PLUGIN_DESCRIPTION "Simple plugin to disarm/smite"
...
public OnPluginStart()
{
	...
	RegAdminCmd("sm_kill", Command_Smite, ADMFLAG_SLAY, "sm_kill <#userid|name> - Slay with a lightning bolt effect.");
...
PerformSmite(client, target)
{
	...
	new wep1 = GetPlayerWeaponSlot(target, CS_SLOT_PRIMARY );
	if( IsValidEdict( wep1 ) )
		{
			RemovePlayerItem(target, wep1);
		}
	new wep2 = GetPlayerWeaponSlot(target, CS_SLOT_SECONDARY );
	if( IsValidEdict( wep2 ) )
		{
			RemovePlayerItem(target, wep2);
		}
	new wep4 = GetPlayerWeaponSlot(target, CS_SLOT_GRENADE	);
	if( IsValidEdict( wep4 ) )
		{
			RemovePlayerItem(target, wep4);
		}		
